// Функция расчета стоимости печати на струйном/сублимационном принтере
insaincalc.calcPrintInkJet = function calcPrintInkJet(numSheet,sizeSheet,quality,color,materialID,printerID,modeProduction = 1) {
    //Входные данные
    //	numSheet - кол-во листов печати
    //	sizeSheet - размер листов, [ширина, высота]
    //  quality - качество печати (стандарт -  0, высокое - 1, фото - 2)
    ...
    try {
        ...
        // расчет печати
        let sheetsPerHour = printer.sheetsPerHour[quality];
        ...
        if (costPrintSheet == 0) {throw (new ICalcError('Не задана или неверно задана цветность печати'))}
        ...
        result.material.set(materialID,[material.name, sizeSheet, Math.ceil(numSheet * (1 + defects))]); //считаем кол-во листов для печати с учетом брака
        return result;
    } catch (err) {
        throw err
    }
