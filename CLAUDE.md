...
Агент **двухэтапный** (по умолчанию `AGENT_USE_ROUTER=true` в `.env`):
1) **Роутер** — классификация `knowledge` | `calculator` + `calculator_slug` (лёгкий контекст: справочник slug/описание/ключевые слова).
2) **Исполнитель** — `knowledge`: только `search_knowledge`; `calculator` + slug: `search_materials` + один `calc_{slug}`; при неясном slug — полный набор tools.
Промпты в `prompts.py`; опциональный алгоритм калькулятора — `GET /api/v1/llm_prompt/{slug}` и `BaseCalculator.get_llm_prompt()`.
При `AGENT_USE_ROUTER=false` — один шаг со всеми tools (как раньше).
...
│ ├── agent.py            — роутер + исполнитель, function calling
│ ├── prompts.py          — промпты роутера, KB, расчёта
...
│ ├── dev.sh                — обёртка для Git Bash, если make не установлен
│ └── wiki_sync.py
│
...
### Локально: API калькуляторов и бот

Из корня репозитория (Python с зависимостями `calc_service` и `bot_service`):

```Bash
make up              # uvicorn + бот (нужен установленный GNU make в PATH)
make down
make status
make restart
make calc-up         # только http://127.0.0.1:8001
make bot-down
```

**Если `make: command not found`** (часто в **Git Bash** на Windows — make не входит в Git for Windows):

```Bash
python scripts/dev.py start all   # универсально
bash scripts/dev.sh start all       # то же из Git Bash
```

Либо установите make, например: `winget install GnuWin32.Make` или `choco install make`, затем перезапустите терминал и снова `make up`.

**Windows cmd/PowerShell** без make: `up.bat`, `down.bat`, `dev.bat status`.
