"""
Калькулятор ЛАМИНАЦИИ ГОТОВЫХ ИЗДЕЛИЙ.

Перенесено из js_legacy/calc/calcLamination.js.
Параметры: тираж, размер изделия, плёнка (laminat), односторонняя/двусторонняя, режим.
Рулонная или пакетная ламинация в зависимости от размера плёнки.

Считает ТОЛЬКО ламинацию уже напечатанных листов/изделий.
Использовать этот калькулятор, когда менеджер просит:
«проламинировать», «сделать только ламинацию» уже существующей продукции.
Если нужно посчитать «листовки с ламинацией» (печать + ламинация),
использовать калькулятор печати листовой продукции (print_sheet).
"""
...
class LaminationCalculator(BaseCalculator):
    """Ламинация ГОТОВЫХ изделий: рулонная или пакетная плёнка по размеру уже напечатанного изделия/листа."""

    slug = "lamination"
    name = "Ламинация"
    description = (
        "Расчёт стоимости ТОЛЬКО ламинации (рулонная или пакетная плёнка) уже напечатанных изделий или листов. "
        "Использовать, когда нужна отдельная услуга ламинации без учёта печати. "
        "Для «листовок с ламинацией» использовать калькулятор печати листовой продукции (print_sheet)."
    )

    def get_param_schema(self) -> Dict[str, Any]:
        """
        Детальная схема параметров для ламинации.
        """
        return {
            "slug": self.slug,
            "title": self.name,
            "params": [
                {
                    ...
                    "required": True,
                    "title": "Тираж",
                    "description": "Количество изделий или листов для ламинации",
                ...
                },
                {
                    ...
                    "required": True,
                    "title": "Ширина (мм)",
                    "description": "Ширина изделия или листа",
                ...
                },
                {
                    ...
                    "required": True,
                    "title": "Высота (мм)",
                    "description": "Высота изделия или листа",
                    "validation": {"min": 10, "max": 1600},
                    "unit": "мм",
                },
                {
                    ...
                    "required": True,
                    "title": "Плёнка",
                    "description": "Плёнка для ламинации",
                    "choices": {
                        "source": "materials:laminat",
                ...
                },
                {
                    ...
                    "required": False,
                    "default": True,
                    "title": "Двусторонняя ламинация",
                    "description": "Если включено — ламинация с двух сторон",
                },
                {
                    "name": "mode",
                    "type": "enum",
