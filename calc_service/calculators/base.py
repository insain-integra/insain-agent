...
class BaseCalculator(ABC):
    ...
    name: str = ""
    description: str = ""
    # DEPRECATED: keywords перенесены на уровень Product (data/products.json).
    # Поле сохранено для обратной совместимости API, но калькуляторы его не заполняют.
    keywords: list[str] = []
    ...
    def get_llm_prompt(self) -> str:
        """
        Дополнительный промпт-алгоритм расчёта для LLM.

        Переопределяется в сложных калькуляторах, где порядок действий нетривиален
        (например, многоуровневый выбор материала, зависимые параметры).
        Базовая реализация возвращает пустую строку — значит спецалгоритма нет.
        """
        return ""
