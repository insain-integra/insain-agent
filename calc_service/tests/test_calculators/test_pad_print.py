"""
Тесты калькулятора тампопечати (pad_print).
"""
...
from calculators.pad_print import PadPrintCalculator
...
def test_share_url(result):
    ...
    assert "pad_print" in result["share_url"]


def test_get_tool_schema(calc):
    schema = calc.get_tool_schema()
    assert schema.get("name") == "calc_pad_print"
...
def test_expected_values_pad_print(ref_result):
    ...
    print("")
    print("  [pad_print] quantity=%s  |  color=%s  |  size_item=%s  |  mode=%s"
          % (REF_PARAMS["quantity"], REF_PARAMS["color"],
             REF_PARAMS["size_item"], REF_PARAMS["mode"]))
    print("  ---")
