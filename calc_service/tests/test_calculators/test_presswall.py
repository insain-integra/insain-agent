"""Тесты калькулятора прессволлов (presswall)."""
...
from calculators.presswall import PresswallCalculator
...
def base_params():
    return {
        "quantity": 1,
        "presswall_id": "Joker30_20_eyelet",
...
def test_share_url(result):
    ...
    assert "presswall" in result["share_url"]


def test_get_tool_schema(calc):
    schema = calc.get_tool_schema()
    assert schema.get("name") == "calc_presswall"
...
def test_expected_values_presswall(ref_result):
    ...
    print("")
    print("  [presswall] Параметры   presswall_id=%s  |  material_id=%s  |  mode=%s"
          % (REF_PARAMS.get("presswall_id"), REF_PARAMS.get("material_id"), REF_PARAMS.get("mode")))
    print("  ---")
