"""
Тесты калькулятора тиснения (embossing).
"""
...
from calculators.embossing import EmbossingCalculator
...
def test_share_url(result):
    ...
    assert "embossing" in result["share_url"]


def test_get_tool_schema(calc):
    schema = calc.get_tool_schema()
    assert schema.get("name") == "calc_embossing"
...
def test_expected_values_embossing(ref_result):
    ...
    ok_mat_name = em["name_substring"] in (mat_match.get("name") or "")
    ok_mat_q = _cmp(float(mat_match.get("quantity") or 0), float(em["quantity_approx"]), 0.15) if mat_match else False

    print("")
    print("  [embossing] quantity=%s  |  type=%s  |  item=%s  |  mode=%s"
          % (REF_PARAMS["quantity"], REF_PARAMS["embossing_type"],
             REF_PARAMS["item_type"], REF_PARAMS["mode"]))
    print("  ---")
