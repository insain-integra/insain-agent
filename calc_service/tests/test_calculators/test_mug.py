...
from calculators.mug import MugCalculator
...
def calc():
    return MugCalculator()


@pytest.fixture(scope="module")
def base_params():
    return {"quantity": 10, "mug_id": "MugStandartWhite", "is_packing": False, "mode": 1}
...
def test_share_url(result):
    ...
    assert "mug" in result["share_url"]
...
def test_get_tool_schema(calc):
    ...
    assert "mug" in schema.get("name", "")
    assert "parameters" in schema


# ── Эталонный тест ───────────────────────────────────────────────────

REF_PARAMS = {"quantity": 10, "mug_id": "MugStandartWhite", "is_packing": False, "mode": 1}
EXPECTED_REF = {"cost": 1732.0, "price": 2827.0, "time_hours": 0.54, "time_ready": 9.077, "weight_kg": 4.13}
EXPECTED_MATERIAL = {"code": "MugStandartWhite", "name_substring": "Кружка", "quantity_approx": 10}
...
def test_expected_values_mug(ref_result):
    ...
    print()
    print("  quantity=%s  mug_id=%s  mode=%s" % (REF_PARAMS["quantity"], REF_PARAMS["mug_id"], REF_PARAMS["mode"]))
    print("  ---")
