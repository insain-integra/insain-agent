"""Тесты калькулятора пазлов (puzzle)."""
...
from calculators.puzzle import PuzzleCalculator
...
def test_share_url(result):
    ...
    assert "puzzle" in result["share_url"]


def test_get_tool_schema(calc):
    schema = calc.get_tool_schema()
    assert schema.get("name") == "calc_puzzle"
...
def test_expected_values_puzzle(ref_result):
    ...
    print("")
    print("  [puzzle] Параметры   puzzle_id=%s  |  mode=%s"
          % (REF_PARAMS.get("puzzle_id"), REF_PARAMS.get("mode")))
    print("  ---")
