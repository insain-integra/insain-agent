"""
Тесты калькулятора УФ-бейджей (uv_badge).
"""
...
from calculators.uv_badge import UVBadgeCalculator


@pytest.fixture(scope="module")
def calc():
    return UVBadgeCalculator()
...
def test_share_url(result):
    ...
    assert "uv_badge" in result["share_url"]
...
def test_get_tool_schema(calc):
    ...
    assert schema.get("name") == "calc_uv_badge"
...
def test_expected_values_uv_badge(ref_result):
    ...
    print("")
    print("  [УФ-бейджи] quantity=%s  |  size=%s×%s  |  material_id=%s  |  color=%s  |  mode=%s"
