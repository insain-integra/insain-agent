"""
Тесты калькулятора холстов (canvas).
"""
...
from calculators.canvas import CanvasCalculator


@pytest.fixture(scope="module")
def calc():
    return CanvasCalculator()
...
def base_params():
    return {
        ...
        "width": 400,
        "height": 300,
        "material_id": "CanvasDLCNM320",
        "printer_code": "HPLatex335",
        "is_frame": True,
        "frame_id": "CanvasFrame4520",
        "mode": 1,
    }
...
def test_share_url(result):
    ...
    assert "canvas" in result["share_url"]


def test_get_tool_schema(calc):
    schema = calc.get_tool_schema()
    assert schema.get("name") == "calc_canvas"
...
EXPECTED_MATERIAL = {
    "code": "CanvasDLCNM320",
    "name_substring": "Холст",
    "quantity_approx": 0.48,
}
...
def test_expected_values_canvas(ref_result):
    ...
    print("")
    print("  [холст] quantity=%s  |  size=%s×%s  |  material_id=%s  |  is_frame=%s  |  mode=%s"
          % (REF_PARAMS["quantity"], REF_PARAMS["width"], REF_PARAMS["height"],
             REF_PARAMS["material_id"], REF_PARAMS["is_frame"], REF_PARAMS["mode"]))
    print("  ---")
